...
step_fetch() {
  ...
  grab "https://github.com/jedisct1/libsodium/releases/download/$SODIUM_VERSION/libsodium-$SODIUM_VERSION.tar.gz" "libsodium-$SODIUM_VERSION.tar.gz" "$SODIUM_SHA256"
...
dep_sodium() { cc; get "https://github.com/jedisct1/libsodium/releases/download/$SODIUM_VERSION/libsodium-$SODIUM_VERSION.tar.gz" "libsodium-$SODIUM_VERSION.tar.gz" "$SODIUM_SHA256"; cd "$WORK/libsodium-$SODIUM_VERSION"; ./autogen.sh; ./configure --prefix="$PREFIX" "${HOSTF[@]}" --enable-static --disable-shared --with-pic; make -j"$NPROC" && make install; }
